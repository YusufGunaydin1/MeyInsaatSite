...
/*
  Satılık kompakt ailesi: iki yoğun landing (A: proje-merkezli, B: vitrin) + iki
  daire detayı — gerçek site kabuğu içinde. Ölçülen şeyler kullanıcının GÖRDÜĞÜ
  davranış: filtre kart sayısını değiştirir, matris lejant+durum yazar, karusel
  sayaç/ok/küçük resim/tam ekran çalışır, ödeme satırları toplama EŞİTTİR,
  ray formu doğrular ve mock sonuç panellerini gösterir, mobilde taşma yoktur.
*/

const K = 'showcases/satilik-daireler/kompakt/';
...
test('vitrin B: proje sekmeleri filtreler; teaser kartlar fiyat uydurmaz', async ({ page }) => {
  await page.goto(u(K + 'vitrin'));
  await expect(page.getByTestId('kc-mock-chip')).toHaveCount(1);
  ...
  await expect(page.getByTestId('kc-tum-daireler')).toHaveAttribute('href', /kompakt\/$/);
});
...
test('detaylar çapraz bağlanır; ölü uçlar dürüst "yakında" diyaloğuna çıkar', async ({ page }) => {
  await page.goto(u(K + 'daire-1'));
  await expect(page.getByTestId('kc-compare')).toHaveAttribute('href', /kompakt\/daire-2$/);
  await expect(page.getByTestId('kc-sim-daire-2')).toHaveAttribute('href', /kompakt\/daire-2$/);
  ...
  // kırıntı geri döner
  await expect(page.getByTestId('kc-breadcrumb').locator('a').nth(1)).toHaveAttribute('href', /kompakt\/$/);
});

test('mobil: dört sayfada yatay taşma yok, görseller çözülür, menü açılır', async ({ page }, testInfo) => {
  test.skip(testInfo.project.name !== 'mobile-360', 'yalnız mobil projede');
  for (const route of ['', 'vitrin', 'daire-1', 'daire-2']) {
    await page.goto(u(K + route));
